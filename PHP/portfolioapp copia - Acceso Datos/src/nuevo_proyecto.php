...
//Convertimos las fechas para poder editarlas
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $clave = isset($_POST['clave']) ? test_input($_POST['clave']) : "";
    $titulo = isset($_POST['titulo']) ? test_input($_POST['titulo']) : "";
    $fecha = isset($_POST['fecha']) ? test_input($_POST['fecha']) : "";
    $descripcion = isset($_POST['descripcion']) ? test_input($_POST['descripcion']) : "";
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] != UPLOAD_ERR_NO_FILE) {
        if ($_FILES['imagen']['error'] != UPLOAD_ERR_OK) {
            $imgErr = "Error al subir imagen, prueba con otra distinta.";
        } else {
        $archivo_tmp = $_FILES['imagen']['tmp_name'];
        $nombre_archivo = $_FILES['imagen']['name'];

        //Comprobar que se una imagen real
        if(getimagesize($archivo_tmp) === false) {
    ...
    if (empty($claveErr) && empty($tituloErr) && empty($descErr) && empty($fechaErr) && empty($imgErr)) {
        ...
        $archivo_json = 'mysql/datos_proyectos.json';
        ...
        <div class="card-body">
            <form action="nuevo_proyecto.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea name="descripcion" class="form-control <?php echo !empty($descErr) ? 'is-invalid' : ''; ?>" rows="4"><?php echo $descripcion; ?></textarea>
                    <?php if (!empty($descErr)) : ?><div class="invalid-feedback"><?php echo $descErr; ?></div><?php endif; ?>
                </div>
